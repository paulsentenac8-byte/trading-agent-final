...
@dataclass
class BrokerConfig:
    # mode: "manual" (CSV à relire), "ibkr" (TWS local requis), "alpaca" (paper trading via HTTP)
    mode: str = "alpaca"
    currency: str = "USD"
    exchange: str = "SMART"

    # Champs IBKR (conservés pour compatibilité, utilisés seulement si mode="ibkr")
    ibkr_host: str = "127.0.0.1"
    ibkr_port: int = 7497
    client_id: int = 7
    account: Optional[str] = None

    # paper_only=True bloque tout envoi réel tant que ce n'est pas explicitement désactivé
    paper_only: bool = True

    # Champs Alpaca — les clés API ne vivent jamais ici, seulement en variables d'environnement
    # (ALPACA_API_KEY / ALPACA_API_SECRET), voir src/alpaca_broker.py
    alpaca_use_bracket_orders: bool = True
    alpaca_close_positions_eod: bool = False
