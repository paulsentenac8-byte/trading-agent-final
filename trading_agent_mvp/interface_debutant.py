...
def check_config() -> list[str]:
    ...
    paper_only = broker.get("paper_only", True)
    mode = broker.get("mode", "manual")
    msgs.append(f"INFO: mode broker actuel -> {mode} ({'démo/paper' if paper_only else 'réel'})")

    if mode == "alpaca":
        has_key = bool(os.environ.get("ALPACA_API_KEY"))
        has_secret = bool(os.environ.get("ALPACA_API_SECRET"))
        if has_key and has_secret:
            msgs.append("OK: clés API Alpaca détectées dans l'environnement.")
        else:
            msgs.append("INFO: clés API Alpaca absentes (ALPACA_API_KEY / ALPACA_API_SECRET). Nécessaire avant tout envoi réel d'ordres.")
